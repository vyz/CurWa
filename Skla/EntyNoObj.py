...
def main():
    listo = gettablolist()
    fo = open("EntyNoObj642.txt", mode='w', encoding='utf-8-sig')
...
def gettablolist():
    global mnm, mtb, mal
    zaprosTablo = """
    SELECT cast(DESCRIBE as nvarchar(100)) as Texo, SOURCE as Tablo, 
    IDN as Alyaso, has_perm_group as g, has_perm_dep as d
    FROM OKP_sysObjects
    ...
    conn = pymssql.connect(host=server, database=dbnameGet,\
           appname='EntyNoObj', conn_properties=ConnPropy)
...
if __name__ == '__main__':
    ...
    logger.info("Begino EntyNoObj")
    main()
    logger.info("Endo EntyNoObj")    